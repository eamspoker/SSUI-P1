...
class FittsTestUI extends UIClass {
    ...
    newTrial() {
        ...
        if (this.trialCount > this.MAX_TRIALS) {
            ...
            this._theReticle.newGeom(targX, targY, targDiam);

            this.configure('begin-trial');
        }
        this.needsRedraw = true;
        this.redraw();
    }
...
class Target extends ScreenObject{
    ...
    newGeom(newCentX : number, newCentY : number, newDiam? : number) {
        ...
        this.w = newDiam ? newDiam : this.w;
        this.h = newDiam ? newDiam : this.h;
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        // draw a target with the specified color, center, and radius
        ctx.beginPath();
        ctx.fillStyle = this.TARGET_COLOR;
        ctx.arc(this.centerX, this.centerY, this.diam/2, 0, 2 * Math.PI);
        ctx.fill();
        ctx.stroke();
    ...
    override pickedBy(ptX : number, ptY : number) : boolean {
        ...
        // use x^2 + y^2 = r^2 formula of circle to determine if it's in the circle
        return (ptX - this.centerX)**2 + (ptY - this.centerY)**2 <= (this.diam/2)**2;
    }
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        // if it's in trial, we record the end and begin a new trial
        if (this.parentUI.currentState == 'in_trial'
            && this.pickedBy(ptX, ptY))
        {
            this.parentUI.recordTrialEnd(ptX, ptY, this.diam/2);
...
class Reticle extends Target {
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        // draw the bigger circle and the inner circle
    ...
    override pickedBy(ptX : number, ptY : number) : boolean {
        // use x^2 + y^2 = r^2 formula of circle to determine if it's in the center region
        return (ptX - this.centerX)**2 + (ptY - this.centerY)**2 <= (Reticle.RETICLE_INNER_DIAM/2)**2;
    }
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        // if the inner area is clicked during the begin_trial phase
        // we record the starting position and move it to the in_trial
        // phase
...
class BackgroundDisplay extends ScreenObject{
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        ctx.fillText(this._msg3, xpos, ypos + (fontHeight*2));
    }
